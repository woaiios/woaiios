...
export class AnalyzedTextComponent extends Component {
     ...
     * 
     * 安全说明 (Security Note):
     * processedText 由 TextAnalyzer.processTextForDisplay() 内部生成，
     * 不直接来自用户输入。该方法会对文本进行适当的转义和处理。
     * (processedText is internally generated by TextAnalyzer.processTextForDisplay(),
     * not directly from user input. The method properly escapes and processes text.)
     */
    render(processedText) {
        // 这里使用 innerHTML 是安全的，因为内容由 TextAnalyzer 生成并经过处理
        // (Using innerHTML here is safe as content is generated and processed by TextAnalyzer)
    ...
    addEventListeners() {
        ...
        this.element.addEventListener('click', (e) => {
            ...
            if (tagName === 'RB' || tagName === 'RT') {
                targetElement = targetElement.parentElement;
            }
            
            // If clicked on ruby or base span inside double-ruby, get the parent double-ruby container
            const parentTagName = targetElement.tagName.toUpperCase();
            if (parentTagName === 'RUBY' || targetElement.classList.contains('base')) {
                targetElement = targetElement.parentElement;
            }
